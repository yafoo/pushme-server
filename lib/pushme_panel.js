const {App, Logger} = require('jj.js');
const {getConfig} = require('./config.js');

/**
 * PushMe 面板服务
 * @description 管理 Web 管理面板和 API 服务
 */
class PushmePanel {
    /**
     * @param {import('./pushme_server.js')} pushme - PushMe服务实例
     * @param {import('./pushme_proxy.js').PushmeProxyInstance} pushmeProxy - PushMe代理对象
     * @param {number} port - 面板端口
     */
    constructor(pushme, pushmeProxy, port) {
        /** @type {import('./pushme_server.js')} */
        this.pushme = pushme;
        /** @type {import('./pushme_proxy.js').PushmeProxyInstance} */
        this.pushmeProxy = pushmeProxy;
        /** @type {number} */
        this.port = port;
        /** @type {import('./config.js').ConfigManager} */
        this.config = getConfig();
        /** @type {import('jj.js').App} */
        this.app = new App();
        /** @type {import('http').Server|import('https').Server|null} */
        this.server = null;
        
        this._init();
    }
    
    /**
     * 初始化
     * @private
     */
    _init() {
        // 将 pushmeProxy 挂载到 Koa 上下文
        this.app.context.pushme = this.pushmeProxy;
        
        // 监听系统重启事件
        this.app.on('systemRestart', async () => {
            Logger.system('System restart event received');
            await this.restart();
        });
    }
    
    /**
     * 面板启动回调
     * @private
     * @param {Error} [err] - 启动错误
     */
    _onListen(err) {
        if(!err) {
            Logger.system('PushMe panel+api is started and listening on port', this.port);
        } else {
            Logger.error('PushMe panel+api start failed, error:', err);
        }
    }
    
    /**
     * 启动面板服务
     */
    start() {
        if(this.config.panelTls == 'tls') {
            this.server = require('https').createServer(this.pushme.tlsOptions, this.app.callback());
            this.server.listen(this.port, this._onListen.bind(this));
        } else {
            this.server = this.app.listen(this.port, this._onListen.bind(this));
        }
    }
    
    /**
     * 重启面板服务（热重启，不退出进程）
     * @returns {Promise<void>}
     */
    async restart() {
        Logger.system('Panel restarting...');
        
        // 关闭旧的服务器
        if(this.server) {
            await new Promise((resolve) => {
                this.server.close(() => {
                    Logger.system('Panel server closed');
                    resolve();
                });
            });
            this.server = null;
        }
        
        // 重新加载配置
        this.config.reload();
        
        // 启动新的服务器
        this.start();
        Logger.system('Panel restarted');
    }
    
    /**
     * 保存消息计数
     */
    saveMessageCount() {
        this.pushmeProxy.messageCountSave();
    }
}

module.exports = PushmePanel;
