...
class ConfigManager {
     ...
     */
    _migrateFromLegacy() {
        Logger.system('检测到旧配置文件，开始迁移到 v1...');
        ...
        if (fs.existsSync(this.oldSettingPath)) {
            try {
                ...
                Logger.system('已迁移 setting.js 配置');
            } catch (err) {
                Logger.system('迁移 setting.js 失败:', err.message);
            }
        }
        ...
        if (fs.existsSync(this.oldDataPath)) {
            try {
                ...
                if (oldData.messageCount !== undefined) {
                    ...
                    Logger.system('已迁移 messageCount:', oldData.messageCount);
                }
            } catch (err) {
                Logger.system('迁移 data.json 失败:', err.message);
            }
        }
        ...
        if (fs.existsSync(this.configPath)) {
            try {
                ...
                Logger.system('读取现有配置失败:', err.message);
            }
        }
        ...
        if (fs.existsSync(this.oldSettingPath)) {
            try {
                ...
                Logger.system('已备份 setting.js 为 setting.js.bak');
            } catch (err) {
                Logger.system('备份 setting.js 失败:', err.message);
            }
        }

        Logger.system('配置迁移到 v1 完成！');
    }
    ...
    _migrateVersion() {
        ...
        try {
            ...
            Logger.system('读取配置文件失败:', err.message);
            return;
        }
        ...
        Logger.system(`配置版本升级: v${currentVersion} -> v${CONFIG_VERSION}`);
    ...
    _saveConfig(config) {
        ...
        try {
            ...
            Logger.system('保存配置失败:', err.message);
    ...
    _load() {
        try {
            ...
            Logger.system('Config load failed:', err.message);
            this._cache = {};
        }
    }
    ...
    _save(config) {
        ...
        try {
            ...
            Logger.system('Config save failed:', err.message);
            // 清理临时文件
            if (fs.existsSync(tempPath)) {
                fs.unlinkSync(tempPath);
            }
    ...
    update(updates) {
        ...
        for (const key of changedKeys) {
            this._notifyChange(key, this._cache[key], oldConfig[key], oldConfig);
        }
    }
    ...
    _notifyChange(key, newValue, oldValue, oldConfig) {
        for (const listener of this._listeners) {
            try {
                ...
                Logger.system('Config change listener error:', err.message);
            }
        }
    }
    ...
    /** @returns {string[]} 推送key列表 */
    get pushKeys() {
        return this._cache.push_keys || [];
    }
    ...
    get status() {
        return this._cache.status || 'stop';
    }
